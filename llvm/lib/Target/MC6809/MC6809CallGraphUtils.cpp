//===-- MC6809CalGraphUtils.cpp - MC6809 Call Graph Utilities -------------===//
//
...
Function *mc6809::getSymbolFunction(Module &M, StringRef Name) {
  Value *V = M.getNamedValue(Name);
  return V ? dyn_cast<Function>(V->stripPointerCastsAndAliases()) : nullptr;
}

void mc6809::addLibcallEdges(CallGraph &CG, const MachineModuleInfo &MMI) {
  for (auto &KV : CG) {
...
void mc6809::addExternalEdges(CallGraph &CG) {
