...
#include "MC6809InlineAsmLowering.h"
#include "llvm/CodeGen/GlobalISel/InlineAsmLowering.h"

using namespace llvm;

MC6809InlineAsmLowering::MC6809InlineAsmLowering(MC6809TargetLowering *TLI) : InlineAsmLowering(TLI) {}
...
static bool handleIntImmediate(Value *Val, StringRef Constraint, std::vector<MachineOperand> &Ops) {
...
bool MC6809InlineAsmLowering::lowerAsmOperandForConstraint(Value *Val, StringRef Constraint, std::vector<MachineOperand> &Ops, MachineIRBuilder &MIRBuilder) const {
  if (handleIntImmediate(Val, Constraint, Ops))
    return true;
  return InlineAsmLowering::lowerAsmOperandForConstraint(Val, Constraint, Ops, MIRBuilder);
}
