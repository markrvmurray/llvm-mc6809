...
;
; 16-bit calling convention validation, run at every optimisation
; level so that opt-level-specific codegen paths can't silently
; diverge from gcc6809 (the reference compiler).
;
; gcc6809 compiles harness-cc16.c into a driver that calls the
; LLVM-compiled cc_funcs16.ll directly. Both halves target ABI
...
; slots, i16 in 2-byte slots). Return i16 in X. The functions here
; take only i16 args, so for f(i16, i16) the first arg fills X
; and the second is pushed via `stx ,--s` (sitting at S+2 after
; jsr). If LLVM-MC6809 diverges from gcc6809 on i16 layout the
; link works but the runtime output mismatches FileCheck — that's
