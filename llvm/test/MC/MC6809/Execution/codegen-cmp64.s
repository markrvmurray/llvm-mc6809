...
; RUN:   %S/Inputs/codegen-cmp64.ll -o %t-raw.s 2>/dev/null
; RUN: grep -v '\.directpage' %t-raw.s > %t-funcs.s
; RUN: %clang6809 -Oz -c -o %t-harness.o %S/Inputs/harness-cmp64.c
...
; RUN: %usim09batch --timeout=2000000 %t.hex | FileCheck %s
;
; 64-bit ICMP execution test. Each row prints six hex digits, one
; per comparison (eq, ne, slt, sgt, ult, ugt) on the same operand
; pair. See harness-cmp64.c for the per-row reasoning.
;
; CHECK: 100000
; CHECK-NEXT: 011010
; CHECK-NEXT: 010101
