...
; RUN:   %S/Inputs/codegen-div64.ll -o %t-raw.s 2>/dev/null
; RUN: grep -v '\.directpage' %t-raw.s > %t-funcs.s
; RUN: %clang6809 -Oz -c -o %t-harness.o %S/Inputs/harness-div64.c
...
; RUN: %usim09batch --timeout=4000000 %t.hex | FileCheck %s
;
...
; RUN: %usim09batch --timeout=4000000 %t.hex | FileCheck %s
;
; 64-bit signed/unsigned division and remainder execution test.
; Exercises divdi.inc (the bug-#100 hand-asm). Larger timeout than
; cmp64 because divdi3 is iterative (64-cycle restoring-division).
;
