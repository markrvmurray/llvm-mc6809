...
; RUN: %clang6809 -Oz -c -o %t-harness.o %S/Inputs/harness-sret.c
...
; REQUIRES: usim
;
; Codegen execution test: struct return via sret pointer (the
; same mechanism used for i32 return after ABI #4). Both gcc6809
; and LLVM-MC6809 use sret-via-IX for any return value larger
; than 2 bytes; the 4-byte struct Pair triggers it. The harness
; (Inputs/harness-sret.c) calls make_pair via the natural C
; struct-return form; the .ll uses an explicit sret(%struct.Pair)
; first parameter.
