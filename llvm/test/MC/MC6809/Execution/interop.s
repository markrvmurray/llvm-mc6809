...
;
; gcc6809 ↔ LLVM-MC6809 mixed-type interop test, run at every
; optimisation level. gcc6809 (the reference compiler) compiles
; harness-interop.c into a driver that calls three LLVM-compiled
; functions covering both register slots of ABI v1: i8 args (filling
; B and 1-byte stack slots) and i16 args (filling X). Both halves
; of the link share the ABI by design — divergence shows up as a
; runtime mismatch against the CHECK lines.
;
; Tests:
